    ...
    var tabler = {
      ...
      '_add_record':function() {
        if(tabler.editor) {
          ...
          elem2 = $('#overlay');
          elem2.append('<div class="outer"><div class="inner"><h4>Add New Record</h4><form class="add-record" id="'+tabler.tableName+'"></form></div></div>');
      ...
      '_get_table_header':function(){
        ...
        for (i=0;i<tabler.header.length;i++)
            $('#'+$(tabler.elem).attr('id')+' thead tr').append('<td><strong>'+tabler.header[i]+'</strong></td>');
        if(tabler.editor)
          $('#'+$(tabler.elem).attr('id')+' thead tr').append('<td><strong>Options</strong></td>');
      },
      ...
      '_show_data':function(){
        ...
        for (i=tabler.pager.startPage;cnt<tabler.pager.rowsPerPage;i++) {
          ...
          if(tabler.data != null && i<tabler.data.length) {
                ...
                if(null!=tabler.protected) {
                  for(x=0;x<tabler.protected.length;x++) {
                    if (tabler.protected[x] == j) {
                      cont += '<td><em>'+tabler.data[i][j]+'</em><input type="hidden" class="column" value="'+j+'"/></td>';
                      break;
                    } else {
                      cont += '<td><span>'+tabler.data[i][j]+'</span><input type="hidden" class="column" value="'+j+'"/></td>';
                      break;
                    }
                  }
                } else {
            ...
            if(tabler.editor)
              cont += '<td><input type="button" class="delete" id="'+tabler.data[i][0]+'" value="" title="Delete"></td></tr>';
          }
        }
      ...
      '_get_data': function() {
        $.ajax({
          type:tabler.methodType,
          async:false,
          url:tabler.dataUrl,
          success:function(resp){
            console.log(resp);
            if(resp != '') {
              tabler.data = $.parseJSON(resp);
            } else {
              tabler.flash('error','<p>There was an issue getting your data.</p>'+resp);
            }
          }
        });
      },
              ...
              if(null!==tabler.fieldTypes && undefined!==tabler.fieldTypes[0]) {
                if(tabler.fieldTypes.length > 0) {
                  for(z=0;z<tabler.fieldTypes.length;z++) {
                    if(tabler.fieldTypes[z][0] == column) {
                      if(tabler.fieldTypes[z][1]=='option') {
                        h = tabler.selectField(column, tabler.value);
                        break;
                      } 
                    } else {
